...
bool IsProcessRunning(const int pid) {
    ...
    HANDLE h_process = OpenProcess(PROCESS_QUERY_INFORMATION | PROCESS_VM_READ | SYNCHRONIZE,
                                   FALSE, pid);
    if (h_process == NULL) {
        DWORD error = GetLastError();
        if (error == ERROR_INVALID_PARAMETER || error == ERROR_ACCESS_DENIED) {
            std::cout << "Process " << pid << " not found or access denied" << std::endl;
            return false;
        }
...
void WaitForProcess(const int pid) {
    ...
    if (attempts >= max_attempts) {
        std::cout << "Process didn't exit naturally, attempting to force close..." << std::endl;
...
int main(const int argc, char **argv) {
    try {
        if (argc < 4) {
            throw std::runtime_error("Usage: Updater <tempExePath> <currentExePath> <pidFilePath>");
        }
        ...
        try {
            ...
            throw std::runtime_error(std::string("Error reading PID file: ") + e.what());
        }
        ...
        try {
            ...
            if (!std::filesystem::exists(current_exe_path)) {
                throw std::runtime_error("Current exe does not exist: " + current_exe_path);
            }
            ...
            throw std::runtime_error(std::string("Filesystem error during update: ") + e.what());
        }
