...
namespace Infinity {
  ...
  ByteVector Encryption::Encrypt(const std::string &plain_text,
    ...
    if (!ctx) {
      Error(ErrorType::Fatal, "Failed to create encryption cipher context")
          .Dispatch();
    }
    ...
    ByteVector ciphertext(plain_text.size() + EVP_MAX_BLOCK_LENGTH +
                          EVP_MAX_IV_LENGTH);
    ...
    if (!EVP_EncryptUpdate(
            ctx, ciphertext.data() + offset, &len,
  ...
  ByteVector Encryption::Encrypt(const ByteVector &binary,
                                 const ByteVector &key) {
    if (key.size() != 32) {
      Error(ErrorType::Fatal, "Attempted encryption with invalid key")
          .Dispatch();
    }
    EVP_CIPHER_CTX *ctx = EVP_CIPHER_CTX_new();
    if (!ctx) {
    ...
    ByteVector ciphertext(binary.size() + EVP_MAX_BLOCK_LENGTH +
                          EVP_MAX_IV_LENGTH);
#ifdef WIN32
    std::ranges::copy(iv, ciphertext.begin());
#else
    ...
    int len;
    if (!EVP_EncryptUpdate(ctx, ciphertext.data() + offset, &len, binary.data(),
  ...
  std::string Encryption::EncryptBase64(const std::string &plain_text,
                                        const ByteVector &key) {
    const auto bytes = Encrypt(plain_text, key);
    return Base64Encode(bytes);
  }

  std::string Encryption::EncryptBase64(const ByteVector &binary,
                                        const ByteVector &key) {
    const auto bytes = Encrypt(binary, key);
    return Base64Encode(bytes);
  }

  std::string Encryption::Decrypt(const ByteVector &binary,
                                  const ByteVector &key) {
    if (key.size() != 32) {
      Error(ErrorType::Fatal, "Attempted decryption with invalid key")
          .Dispatch();
    }
    ...
    if (!EVP_DecryptUpdate(
            ctx, plain_text.data(), &len, binary.data() + EVP_MAX_IV_LENGTH,
            static_cast<int>(binary.size()) - EVP_MAX_IV_LENGTH)) {
      EVP_CIPHER_CTX_free(ctx);
  ...
  ByteVector Encryption::DecryptToBin(const ByteVector &binary,
                                      const ByteVector &key) {
    if (key.size() != 32) {
      Error(ErrorType::Fatal, "Attempted decryption with invalid key")
          .Dispatch();
    ...
    if (!EVP_DecryptUpdate(
            ctx, plaintext.data(), &len, binary.data() + EVP_MAX_IV_LENGTH,
            static_cast<int>(binary.size() - EVP_MAX_IV_LENGTH))) {
      EVP_CIPHER_CTX_free(ctx);
  ...
  std::string Encryption::DecryptBase64(const std::string &base64,
                                        const ByteVector &key) {
    const auto bytes = Base64Decode(base64);
    return Decrypt(bytes, key);
  }

  ByteVector Encryption::DecryptBase64Bin(const std::string &base64,
                                          const ByteVector &key) {
    const auto bytes = Base64Decode(base64);
  ...
  ByteVector Encryption::Base64Decode(const std::string &base64_data) {
    ...
    const int decoded_size =
        BIO_read(bio, result.data(), static_cast<int>(base64_data.size()));
    BIO_free_all(bio);
    if (decoded_size < 0) {
