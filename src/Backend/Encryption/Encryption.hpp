...
public:
    ...
    [[maybe_unused]] static ByteVector Encrypt(const std::string &plain_text,
                                               const ByteVector &key);
    [[maybe_unused]] static ByteVector Encrypt(const ByteVector &binary,
                                               const ByteVector &key);
    [[maybe_unused]] static std::string EncryptBase64(
        const std::string &plain_text, const ByteVector &key);
    [[maybe_unused]] static std::string EncryptBase64(const ByteVector &binary,
                                                      const ByteVector &key);

    static std::string Decrypt(const ByteVector &binary, const ByteVector &key);
    static ByteVector DecryptToBin(const ByteVector &binary,
                                   const ByteVector &key);
    static std::string DecryptBase64(const std::string &binary,
                                     const ByteVector &key);
    static ByteVector DecryptBase64Bin(const std::string &binary,
                                       const ByteVector &key);
...
private:
  ...
  inline ByteVector CreateEncryptedKey(const unsigned char *raw_bin,
                                       const ByteVector &key) {
