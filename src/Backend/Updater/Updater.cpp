...
namespace Infinity {
  ...
  void Updater::LaunchUpdater(const std::string &updater_path,
    ...
    command = "start \"\" \"" + win_updater_path + "\" \"" + win_temp_exe_path +
        "\" \"" + win_current_exe_path + "\" \"" + win_pid_file_path + "\"";
#else
    command = "\"" + updater_path + "\" \"" + temp_exe_path + "\" \"" +
        current_exe_path + "\" \"" + pid_file_path +
  ...
  std::string Updater::GetConfigDir() {
    ...
    size_t len = 0;
    if (_dupenv_s(&appdata_path, &len, "APPDATA") != 0 ||
        appdata_path == nullptr) {
      std::cerr << "Error: Unable to determine AppData folder (APPDATA "
    ...
    if (home_dir == nullptr) {
      std::cerr << "Error: Unable to determine home directory (HOME "
                   "environment variable is not set)."
                << std::endl;
    } else {
      output_path =
          std::filesystem::path(home_dir) / ".config/infinity-launcher";
    }
#else
