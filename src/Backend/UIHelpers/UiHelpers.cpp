...
namespace Infinity {
  ...
  void DrawButtonImage(const std::shared_ptr<Image> &imageNormal,
                       ...
                       const ImU32 tintNormal, const ImU32 tintHovered,
                       const ImU32 tintPressed, const ImVec2 rectMin,
                       const ImVec2 rectMax) {
    auto *drawList = ImGui::GetForegroundDrawList();
    if (ImGui::IsItemActive())
      drawList->AddImage(imagePressed->GetImGuiTextureID(), rectMin, rectMax,
  ...
  void RenderWindowOuterBorders(ImGuiWindow *window) {
    ...
    auto GetResizeBorderRect = [](ImGuiWindow *window, const int border_n,
                                  const float perp_padding,
  ...
  bool BeginMenubar(const ImRect &barRectangle) {
    ...
    ImRect bar_rect =
        RectOffset(barRectangle, 0.0f, padding.y);  // window->MenuBarRect();
    ImRect clip_rect(
        IM_ROUND(ImMax(
            window->Pos.x,
        ...
        IM_ROUND(
            ImMax(bar_rect.Min.x + window->Pos.x,
                  bar_rect.Max.x -
