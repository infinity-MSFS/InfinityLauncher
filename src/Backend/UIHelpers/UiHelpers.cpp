...
namespace Infinity {
  ...
  void DrawButtonImage(const std::shared_ptr<Image> &imageNormal, const std::shared_ptr<Image> &imageHovered,
    ...
    if (ImGui::IsItemActive())
      drawList->AddImage(imagePressed->GetImGuiTextureID(), rectMin, rectMax, ImVec2(0, 0), ImVec2(1, 1), tintPressed);
    else if (ImGui::IsItemHovered())
      drawList->AddImage(imagePressed->GetImGuiTextureID(), rectMin, rectMax, ImVec2(0, 0), ImVec2(1, 1), tintHovered);
    else
      drawList->AddImage(imagePressed->GetImGuiTextureID(), rectMin, rectMax, ImVec2(0, 0), ImVec2(1, 1), tintNormal);
  }
  ...
  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svgNormal, const std::shared_ptr<SVGImage> &svgHovered,
    ...
    if (ImGui::IsItemActive()) {
      SVGImage::RenderSVG(svgPressed, rectMin, size);
      drawList->AddRectFilled(rectMin, rectMax, tintPressed & 0x7FFFFFFF);
    } else if (ImGui::IsItemHovered()) {
      SVGImage::RenderSVG(svgHovered, rectMin, size);
      drawList->AddRectFilled(rectMin, rectMax, tintHovered & 0x7FFFFFFF);
    } else {
      SVGImage::RenderSVG(svgNormal, rectMin, size);
      drawList->AddRectFilled(rectMin, rectMax, tintNormal & 0x7FFFFFFF);
    }
  }
  ...
  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svgNormal, const std::shared_ptr<SVGImage> &svgHovered,
                          const std::shared_ptr<SVGImage> &svgPressed, const ImU32 tintNormal, const ImU32 tintHovered,
                          const ImU32 tintPressed, const ImRect rectangle) {
    DrawSVGButtonImage(svgNormal, svgHovered, svgPressed, tintNormal, tintHovered, tintPressed, rectangle.Min,
                       rectangle.Max);
  }

  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svg, const ImU32 tintNormal, const ImU32 tintHovered,
                          const ImU32 tintPressed, const ImVec2 rectMin, const ImVec2 rectMax) {
    DrawSVGButtonImage(svg, svg, svg, tintNormal, tintHovered, tintPressed, rectMin, rectMax);
  }

  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svg, const ImU32 tintNormal, const ImU32 tintHovered,
                          const ImU32 tintPressed, const ImRect rectangle) {
    DrawSVGButtonImage(svg, svg, svg, tintNormal, tintHovered, tintPressed, rectangle.Min, rectangle.Max);
  }

  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svgNormal, const std::shared_ptr<SVGImage> &svgHovered,
                          const std::shared_ptr<SVGImage> &svgPressed, const ImU32 tintNormal, const ImU32 tintHovered,
                          const ImU32 tintPressed) {
    DrawSVGButtonImage(svgNormal, svgHovered, svgPressed, tintNormal, tintHovered, tintPressed, ImGui::GetItemRectMin(),
                       ImGui::GetItemRectMax());
  }

  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svg, const ImU32 tintNormal, const ImU32 tintHovered,
                          const ImU32 tintPressed) {
    DrawSVGButtonImage(svg, svg, svg, tintNormal, tintHovered, tintPressed, ImGui::GetItemRectMin(),
                       ImGui::GetItemRectMax());
  }
  ...
  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svgNormal, const std::shared_ptr<SVGImage> &svgHovered,
    ...
    if (ImGui::IsItemActive()) {
      auto svgCopy = svgPressed->Clone();
      svgCopy->ChangeColor(colorPressed);
      SVGImage::RenderSVG(svgCopy, rectMin, size);
    } else if (ImGui::IsItemHovered()) {
      auto svgCopy = svgHovered->Clone();
      svgCopy->ChangeColor(colorHovered);
      SVGImage::RenderSVG(svgCopy, rectMin, size);
    } else {
      auto svgCopy = svgNormal->Clone();
      svgCopy->ChangeColor(colorNormal);
      SVGImage::RenderSVG(svgCopy, rectMin, size);
    }
  }
  ...
  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svg, const std::string &colorNormal,
                          const std::string &colorHovered, const std::string &colorPressed, const ImVec2 rectMin,
                          const ImVec2 rectMax) {
    DrawSVGButtonImage(svg, svg, svg, colorNormal, colorHovered, colorPressed, rectMin, rectMax);
  }

  void DrawSVGButtonImage(const std::shared_ptr<SVGImage> &svg, const std::string &colorNormal,
                          const std::string &colorHovered, const std::string &colorPressed) {
    DrawSVGButtonImage(svg, svg, svg, colorNormal, colorHovered, colorPressed, ImGui::GetItemRectMin(),
                       ImGui::GetItemRectMax());
  }
  ...
  bool UpdateWindowManualResize(ImGuiWindow *window, ImVec2 &newSize, ImVec2 &newPosition) {
    ...
    auto CalcWindowAutoFitSize = [CalcWindowSizeAfterConstraint](ImGuiWindow *window, const ImVec2 &size_contents) {
      ...
      if (window->Flags & ImGuiWindowFlags_Tooltip) {
        ...
        if (is_popup || is_menu)
          // Popups and menus bypass style.WindowMinSize by default, but we give
    ...
    if (!handle_borders_and_resize_grips || window->Collapsed) return false;
    ...
    auto CalcResizePosSizeFromAnyCorner = [CalcWindowSizeAfterConstraint](
                                              ImGuiWindow *window, const ImVec2 &corner_target,
    ...
    static const float WINDOWS_HOVER_PADDING = 4.0f;  // Extend outside window for hovering/resizing (maxxed with
                                                      // TouchPadding) and inside windows for
    // borders. Affect FindHoveredWindow().
    static const float WINDOWS_RESIZE_FROM_EDGES_FEEDBACK_TIMER = 0.04f;  // Reduce visual noise by only highlighting
                                                                          // the border after a certain time.
  ...
  bool BeginMenubar(const ImRect &barRectangle) {
    ...
    ImGui::BeginGroup();  // Backup position on layer 0 // FIXME: Misleading to
  ...
  void EndMenubar() {
    ...
    if (ImGui::NavMoveRequestButNoResultYet() && (g.NavMoveDir == ImGuiDir_Left || g.NavMoveDir == ImGuiDir_Right) &&
      ...
      if (nav_earliest_child->ParentWindow == window &&
        ...
        // delay which isn't very problematic in this situation. We could remove
        // it by scoring in advance for multiple window (probably not worth
        // bothering)
    ...
    IM_MSVC_WARNING_SUPPRESS(6011);  // Static Analysis false positive "warning C6011: Dereferencing
                                     // NULL pointer 'window'"
    // IM_ASSERT(window->Flags & ImGuiWindowFlags_MenuBar); // NOTE(Yan): Needs
    // to be commented out because Jay
    IM_ASSERT(window->DC.MenuBarAppending);
    ImGui::PopClipRect();
    ImGui::PopID();
