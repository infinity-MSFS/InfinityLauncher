...
namespace ImGui {
    TextRegion::TextRegion() : indent_x(0.0f) {}
    ...
    bool TextRegion::RenderLinkText(const char *text, const char *text_end, const Link &link, const char *markdown, const MarkdownConfig &markdown_config, const char **link_hover_start) {
        ...
        if (hovered) {
            if (IsMouseReleased(0) && markdown_config.link_callback) {
                markdown_config.link_callback({markdown + link.text.start, link.text.size(), markdown + link.url.start, link.url.size(), markdown_config.user_data, false});
            }
            if (markdown_config.tooltip_callback) {
                markdown_config.tooltip_callback(
                        {{markdown + link.text.start, link.text.size(), markdown + link.url.start, link.url.size(), markdown_config.user_data, false}, markdown_config.link_icon});
            }
        }
        return this_item_hovered;
    }

    bool IsCharInWord(char c) { return c != ' ' && c != '.' && c != ',' && c != ';' && c != '!' && c != '?' && c != '\"'; }

    void TextRegion::RenderLinkTextWrapped(const char *text, const char *text_end, const Link &link, const char *markdown, const MarkdownConfig &markdown_config, const char **link_hover_start,
                                           bool indent_to_here) {
                ...
                case Link::HAS_SQUARE_BRACKETS_ROUND_BRACKET_OPEN:
                    ...
                    if (link.num_brackets_open == 0) {
                        ...
                        if (link.is_image) {
                            ...
                            textRegion.RenderLinkTextWrapped(markdown + link.text.start, markdown + link.text.start + link.text.size(), link, markdown, markdown_config, &linkHoverStart, false);
                        }
                        ImGui::SameLine(0.0f, 0.0f);
                        link = Link();
            ...
            switch (em.state) {
                case Emphasis::NONE:
                    if (link.state == Link::NO_LINK && !line.is_heading) {
                        int next = i + 1;
                        int prev = i - 1;
                        if ((c == '*' || c == '_') && (i == line.line_start || markdown[prev] == ' ' || markdown[prev] == '\t') && static_cast<int>(markdown_length) > next && markdown[next] != ' ' &&
                            markdown[next] != '\n' && markdown[next] != '\t') {
...
namespace Infinity {
    ...
    Markdown::Markdown() : m_Config({LinkCallback, nullptr, ICON_FA_LINK, {{Application::GetFont("h1"), true}, {Application::GetFont("h2"), true}, {Application::GetFont("h3"), true}}, nullptr}) {}
