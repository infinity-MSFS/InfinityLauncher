...
namespace Infinity {
  ...
  void Home::Render() {
    ...
    ImGui::Text("Infinity");
    ImGui::PopFont();
    size_t index = 4;  // skip reserved pages
  ...
  void Home::RegisterProject(const std::string &name, std::shared_ptr<Image> image, std::shared_ptr<Image> logo,
                             const int page_id) {
  ...
  void Home::RenderProject(const HomeProjectButtonStruct &project, const int page_index) {
    ...
    const float x_pos = base_x - base_x + 30.0f + static_cast<float>(page_index - 4) * base_x - m_ScrollOffset;
    ...
    const auto logo_position =
        ImVec2(x_pos + size.x / 2 - logo_size.x / 2, ImGui::GetWindowHeight() - logo_size.y - 150.0f);
