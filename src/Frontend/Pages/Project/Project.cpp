...
namespace Infinity {
  ...
  std::shared_ptr<uint8_t> ProjectPage::m_SelectedAircraft =
  ...
  AircraftSelectButton::AircraftSelectButton(
  ...
  bool AircraftSelectButton::Button(const char *label, ImVec2 size,
                                    bool active) {
    ...
    const ImVec2 active_arrow_location(
        cursor_pos.x + size.x / 2 - active_arrow_size.x / 2,
        cursor_pos.y + size.y);


    if (active) {
      draw_list->AddRectFilled(
          cursor_pos, ImVec2(cursor_pos.x + size.x, cursor_pos.y + size.y),
          ImGui::GetColorU32(bg_color), 2.0f);
      draw_list->AddTriangleFilled(
          active_arrow_location,
          ImVec2(active_arrow_location.x + active_arrow_size.x,
                 active_arrow_location.y),
          ImVec2(active_arrow_location.x + active_arrow_size.x / 2,
                 active_arrow_location.y + active_arrow_size.y),
    ...
    ImVec2 textPos = ImVec2(cursor_pos.x + (size.x - textSize.x) * 0.5f,
                            cursor_pos.y + (size.y - textSize.y) * 0.5f);
    draw_list->AddText(textPos, ImGui::GetColorU32(text_color), label);

    return pressed;
  }

  AircraftSelectButtonBar::AircraftSelectButtonBar(
  ...
  void AircraftSelectButtonBar::Render() {
    ...
    constexpr float padding =
        40.0f;  // padding on left and right side of screen
    constexpr float gap = 10.0f;  // gap between buttons
    const float button_width =
        (ImGui::GetWindowWidth() - padding * 2 - gap * (button_count - 1)) /
        button_count;
  ...
  TopRegion::TopRegion(
    ...
    for (int32_t i = 0; i < group_data->projects.size(); i++) {
      m_AircraftSelectButtons.emplace_back(AircraftSelectButton(
          group_data->projects[i].name, i, selected_aircraft));
    }
    m_AircraftSelectButtonBar =
        AircraftSelectButtonBar(m_AircraftSelectButtons, selected_aircraft);
  }
  ...
  void TopRegion::Render() {
    ...
    Image::RenderImage(m_GroupDataImages->logo,
                       ImVec2(ImGui::GetWindowWidth() / 2 - text_size.x / 2 -
                                  logo_size.x / 2 - gap_text_logo,
                              ImGui::GetWindowHeight() / 8 - logo_size.y / 2),
                       logo_size);
    ImGui::SetCursorPos(ImVec2(ImGui::GetWindowWidth() / 2 - text_size.x / 2 +
                                   logo_size.x / 2 + gap_text_logo,
  ...
  ContentRegionButton::ContentRegionButton(
      const std::string &name, int8_t id,
      const std::shared_ptr<uint8_t> &selected_page)
      : m_ButtonSpec(name, id)
      , m_SelectedPage(selected_page) {}

  ContentRegionButton::ContentRegionButton(
      const Button &button, const std::shared_ptr<uint8_t> &selected_page)
      : m_ButtonSpec(button)
      , m_SelectedPage(selected_page) {}


  void ContentRegionButton::Render(ImVec2 size, ImVec2 pos) {
    if (ButtonRender(m_ButtonSpec.name.c_str(), size,
                     *m_SelectedPage == m_ButtonSpec.page)) {
      *m_SelectedPage = m_ButtonSpec.page;
    }
  }

  bool ContentRegionButton::ButtonRender(const char *label, ImVec2 size,
                                         bool active) {
    auto cursor_pos = ImGui::GetCursorScreenPos();
    ...
    if (active) {
      draw_list->AddLine({cursor_pos.x, cursor_pos.y + size.y},
                         {cursor_pos.x + size.x, cursor_pos.y + size.y},
  ...
  ContentRegionButtonBar::ContentRegionButtonBar(
  ...
  void ContentRegionButtonBar::Render() {
    ...
    const float button_width =
        (ImGui::GetWindowWidth() - padding * 2 - gap * (m_Buttons.size() - 1)) /
        m_Buttons.size();
    const ImVec2 button_size(button_width, 50.0f);
    for (auto &button: m_Buttons) {
      ImGui::SetCursorPos({padding + index * (button_width + gap),
                           ImGui::GetWindowHeight() / 3.0f + 130.0f});
      button.Render(button_size,
                    ImVec2(padding + index * (button_width + gap),
                           ImGui::GetWindowHeight() / 3.0f));
      index++;
    }
  }
  ...
  ContentRegion::ContentRegion(
    ...
    auto description_button =
        ContentRegionButton("Description", 1, m_SelectedPage);
    auto changelog_button = ContentRegionButton("Changelog", 2, m_SelectedPage);
    m_ButtonBar = ContentRegionButtonBar(
        {overview_button, description_button, changelog_button},
        m_SelectedPage);
  }

  void ContentRegion::RenderInstalledWidget() {
    ImGui::GetWindowDrawList()->AddRectFilled(
        {40.0f, ImGui::GetWindowHeight() / 3.0f + 10.0f},
        {200.0f, ImGui::GetWindowHeight() / 3.0f + 40.0f},
        ImGui::GetColorU32(ImVec4(0.0f, 0.0f, 0.0f, 0.5f)), 10.0f);

    ImGui::GetWindowDrawList()->AddText(
        {45.0f, ImGui::GetWindowHeight() / 3.0f + 10.0f + 5.0f},
        ImGui::GetColorU32(ImVec4(1.0f, 1.0f, 1.0f, 0.6f)), "Not Installed");
  }

  bool ContentRegion::RenderDownloadButton(const char *label, ImVec2 size,
                                           ImVec2 pos) {
    ImGui::SetCursorPos(pos);
  ...
  void ContentRegion::Render() {
    ...
    auto download_label = "Download Version " +
        m_GroupData->projects[*m_SelectedAircraft].version;
    if (RenderDownloadButton(
            download_label.c_str(), {200.0f, 60.0f},
            {ImGui::GetWindowWidth() - 100.0f - 60.0f - 10.0f - 200.0f,
             ImGui::GetWindowHeight() / 3.0f + 50.0f})) {
      std::cout << "Download button pressed for: "
                << m_GroupData->projects[*m_SelectedAircraft].name << std::endl;
    }
    if (RenderBugReportButton({120.0f, 60.0f},
                              {ImGui::GetWindowWidth() - 60.0f - 100.0f,
                               ImGui::GetWindowHeight() / 3.0f + 50.0f})) {
      std::cout << "Bug report button pressed for: "
                << m_GroupData->projects[*m_SelectedAircraft].name << std::endl;
    }

    ImGui::SetCursorPos({40.0f, ImGui::GetWindowHeight() / 3.0f + 50.0f});
    ImGui::PushFont(Application::GetFont("BoldXLarge"));
    const auto header_size = ImGui::CalcTextSize(
        m_GroupData->projects[*m_SelectedAircraft].name.c_str());
    ...
    switch (*m_SelectedPage) {
      case 0: {
        ImGui::Text(
            "Overview: %s",
            m_GroupData->projects[*m_SelectedAircraft].overview.c_str());
        break;
      }
      case 1: {
        Markdown::GetInstance()->Render(
            m_GroupData->projects[*m_SelectedAircraft].description);
      ...
      case 2: {
        ImGui::Text(
            "Changelog: %s",
            m_GroupData->projects[*m_SelectedAircraft].changelog.c_str());
        break;
      }
      default:
        break;
